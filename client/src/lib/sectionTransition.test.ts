...
import { MENU_SECTION_LOADING_DURATION_MS, MENU_SECTION_REVEAL_DELAY_MS, shouldRunMenuTransition } from "./sectionTransition";

describe("section transition policy", () => {
  it("uses a compact mission-loading envelope before revealing a new menu section", () => {
    expect(MENU_SECTION_LOADING_DURATION_MS).toBe(680);
